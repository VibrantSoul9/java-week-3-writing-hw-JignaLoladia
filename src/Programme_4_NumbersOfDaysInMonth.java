 ...
 * if the parameter is not in that range retrun false.
 * otherwise, if it is in the valid range, calculate if the year is a leap year is a leap year and return true if it is,
 * otherwise return false
 * A year is a leap year if it is divisible by 4 but not by 100, or it is divisible by 400
 * Example of input/output:
 *  ● isLeapYear(-1600); should return false since the parameter is not in the range (1-9999) ●
 *  isLeapYear(2017); should return flase since 2017 is not a leap year
 *  ● isLeapYear(2000); should return true because 2000 is a leap year
 *  Write another method getDaysInMonth with two parameter month and year
 *  Both of typ int
 *  if parameter month is &lt; 1 or &gt; 12 retrun -1
 *  if parameter year is &lt; or &gt: 9999 than return -1
 *  This method needs to return the number of days in the month
 *  be careful about leap years
 ...
 * ● getDaysInMonth(2, 2018); should return 28 since February has 28 days if its not a leap year and 2018 is not a leap year
 * ● getDaysInMonth(-1, 2020); should return -1 since the parameter month is invalid
 * ● getDaysInMonth(1,-2020); should return -1 since the parameter year is outside the range of 1 to 9999
 * Hint : Use the Swith statement
...
public class Programme_4_NumbersOfDaysInMonth {
    ...
    public static boolean isLeapYear(int years) {
        ...
        if (years <= 1 || years >= 9999) {
            b = false;
        } else { b = years % 4 == 0 && years % 100 != 0 || years % 400 == 0;
        }
        return b;
    }
    // calculalting the days of the month depends on year
